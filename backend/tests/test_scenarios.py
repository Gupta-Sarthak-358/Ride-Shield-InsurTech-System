...
async def create_worker_policy(client, name: str, phone: str, zone: str, income: int, plan_name: str, profile: str) -> str:
...
@pytest.mark.asyncio
async def test_legitimate_rain_scenario_auto_approves(client):
...
@pytest.mark.asyncio
async def test_fraud_rain_scenario_is_not_auto_approved(client):
    worker_id = await create_worker_policy(
        client,
        "Vikram Singh",
        "+919111111112",
...
@pytest.mark.asyncio
async def test_edge_platform_outage_scenario_routes_to_review_or_reject(client):
    worker_id = await create_worker_policy(
        client,
        "Arun Patel",
        "+919111111113",
